...
public class SuspiciousWebPattern implements IPattern {
	 ...
	 * IPs on blacklisted ports. Severity is determined based on the number of
	 * blacklisted requests made
	 * 
	 * @param log
	 *            The EventLog to check through
	 * 
	...
	public LinkedList<Alert> run(EventLog log, LinkedList<String> usernames) {
		...
		for (String uname : usernames) {
			// Track the number of requests made by user to blacklisted
			// addresses
			int count = 0;
			...
			for (AbsEvent event : log.getLog()) {
				...
				// The AbsEvent class defines various kinds of events (such as
				// WEB_REQUEST),
				...
				if (event.isByUser(uname) && (event.getType() == AbsEvent.WEB_REQUEST)) {
					...
					// --
					// this is just using data from the request event -- details
					// not inmportant
			...
			// if we detected any suspicious web requests, create an alert for
			// this user
			if (count > 0) {
				int severity = (Integer) Math.min(10, (count / 1));
				Alert a = new Alert(uname, severity, AbsEvent.WEB_REQUEST);
				raiseAlerts.add(a);
				// raiseAlerts.add(new Alert(uname, severity,
				// AbsEvent.WEB_REQUEST));
			}
		}
		return raiseAlerts;
	}
